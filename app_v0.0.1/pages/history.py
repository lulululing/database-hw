...
def show():
    """显示历史数据查询页面"""
    st.markdown('<div class="main-header">📊 历史销售数据查询</div>', unsafe_allow_html=True)
            ...
            if 'export' in st.session_state.permissions:
                ...
                with col2:
                    ...
                    st.download_button(
                        label="📥 导出为 CSV",
                        data=csv,
                        file_name=f"历史数据_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv",
                        use_container_width=True
                    )
