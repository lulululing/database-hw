...
# 检查权限 - 扩展：业务员也能查看
view_perms = ['view_budget', 'view_budget_country']
...
# 设置页面
st.set_page_config(
    page_title=get_text('nav_budget'),
    layout="wide"
)
                    ...
                    if 'export' in permissions:
                        ...
                        with col2:
                            ...
                            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                                df_converted.to_excel(writer, index=False, sheet_name='Budget_Data')
                            ...
                            st.download_button(
                                ...
                                data=excel_data,
                                file_name=f"budget_data_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
